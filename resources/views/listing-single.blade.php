                ...
                <div class="col-md-10" data-aos="fade-up" data-aos-delay="400">
                    ...
                    <div class="row justify-content-center">
                        <div class="col-md-8 text-center">
                            ...
                            <p class="mb-0"><i class="icon icon-room" style="font-size: 30px"></i>
                                {{ $entInfo[0]->entDescripPlace }}</p>
                        </div>
                    </div>
...
@section('pageContent')
                            ...
                            @foreach ($produits as $produit)
                                <div class="col-md-6">
                                    <button type="button" style="border: none; outline: none; cursor: pointer"
                                        class="popular-category h-100" data-toggle="modal"
                                        data-target="#orderModal{{ $produit->prodId }}">
                                        <span class="icon mb-3">
                                            <img src="{{ asset(__('storage/') . $produit->prodImg) }}"
                                                alt="{{ $produit->marqLib }}" class="img-fluid mb-4">
                                        </span>
                                        <span class="caption mb-2 d-block">{{ $produit->marqLib }}
                                            ({{ $produit->prodCat }})</span>
                                        Rechargement: <span
                                            class="number">{{ number_format($produit->puRechargement, 0, ',', '.') }}
                                            frcfa</span>
                                        <br>
                                        <br>
                                        Nouvelle bouteille: <span
                                            class="number">{{ number_format($produit->puAchat, 0, ',', '.') }}
                                            frcfa</span>
                                    </button>
                                </div>
                                <!-- Modal -->
                                <div class="modal fade" id="orderModal{{ $produit->prodId }}" tabindex="-1" role="dialog"
                                            ...
                                            <form action="{{ __('/oderProd') }}" method="post">
                                                ...
                                                <div class="modal-body">
                                                    @if (!Auth::check())
                                                        <div class="alert alert-info">
                                                            Connectes-toi avant de passer une commande. <a
                                                                href="{{ __('/Se-connecter') }}">Me
                                                                connecter</a>
                                                        </div>
                                                    @endif
                                                    ...
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input" required type="radio"
                                                            name="livraison" id="livraisonOui{{ $produit->prodId }}"
                                                            value="oui"
                                                    ...
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input" required type="radio"
                                                            name="livraison" id="livraisonNon"
                                                    ...
                                                    <div id="deliveryDay{{ $produit->prodId }}" style="display: none">
                                                        ...
                                                        <div class="form-check form-check-inline">
                                                            <input class="form-check-input" type="date" name="dateLivraison"
                                                    ...
                                                    @endif
                                                    <input class="form-check-input form-control" type="hidden" name="entId"
                                                        value="{{ $produit->entId }}">
                                                    <input class="form-check-input form-control" type="hidden" name="prodId"
                                                        value="{{ $produit->prodId }}">
                                                </div>
                                                <div class="modal-footer">
                                                    ...
                                                    <button type="submit" @if (!Auth::check()) disabled @endif
                                                        class="btn btn-primary">Commander <i
                ...
                <div class="col-lg-3 ml-auto">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="form-group">
                            <i class="icon icon-user" style="font-size: 30px; color: #00918e"></i>
                            {{ $entInfo[0]->pnom . ' ' . $entInfo[0]->nom }}
                            <br>
                            <i class="icon icon-phone_android" style="font-size: 30px; color: #00918e"></i> <a
                                href="tel:{{ $entInfo[0]->tel }}"> {{ $entInfo[0]->tel }}</a>
                            <br>
                            <i class="icon icon-room" style="font-size: 30px; color: #00918e"></i>
                            {{ $entInfo[0]->comLib . ', ' . $entInfo[0]->entDescripPlace }}
                            <br>
                            <br>
                            <!--Geoloc here-->
    ...
    <script type="text/javascript">
        function livraisonFunction(globalField, idField) {
            ...
            else if (deliveryDayField.checked == false) {
